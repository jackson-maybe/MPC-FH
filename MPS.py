...
def rightmost_binary_1_position(num):
    ...
    return i + 1
f=open('Input-P0-0', 'w')
def MPS(numbers, Fu, bits_for_bucket_index, seed_bias=0):
    bucket_count = 2 ** bits_for_bucket_index
    seed_bias = seed_bias * bucket_count
    buckets_A = [[int(0) for column_index in range(25)] for row_index in range(bucket_count) ]
    buckets_B = [[int(0) for column_index in range(25)] for row_index in range(bucket_count) ]
    buckets_C = [[int(0) for column_index in range(25)] for row_index in range(bucket_count) ]
    buckets_D = [[int(0) for column_index in range(25)] for row_index in range(bucket_count) ]
    for bucket_num in range(bucket_count):
        for v_num, v in enumerate(numbers):
            w = hash(v, seed=bucket_num + seed_bias)
            buckets_A[bucket_num][rightmost_binary_1_position(w) - 1] += v
            buckets_B[bucket_num][rightmost_binary_1_position(w) - 1] += int(v) * int(v)
            buckets_C[bucket_num][rightmost_binary_1_position(w) - 1] += 1
            buckets_D[bucket_num][rightmost_binary_1_position(w) - 1] += Fu[v_num]
...
def experiment_MPS(num_elements, provider_data_num, distribution,epsilon,delta, num_trials=1,
    ...
    num_prop_pre_all = []
    max_fre = 15
    sensitivity = pow(2, m_bit)
    sketch_type = 'MPS'
    sigma2 = searching_sigma(epsilon, delta,sensitivity, num_provider)
    for trial in range(num_trials):
        if distribution == 'homogeneous':
            raw_data_frequency = [[create_equal_independent_data_set(num_elements, provider_data_num), create_homogeneous_frequency_set(provider_data_num)]  for index in range(num_provider)]
        else:
            raw_data_frequency = [[create_equal_independent_data_set(num_elements, provider_data_num), create_heterogeneous_frequency_set(provider_data_num)] for index in range(num_provider)]
        ...
        for provider_id in range(num_provider):
            ...
            sketch_list = MPS(data_pre[:,0], data_pre[:,1], m_bit)
            ...
            distributed_sketch_A_df.to_pickle(
                f'./distributed_sketch_data/mbit={m_bit}_sketchtype={sketch_type}_proportion={proportion}_numprovider={num_provider}_{distribution}_numelement={num_elements}_distributed_sketchA_provider={provider_id}_trial={trial}_experimentnumber={experiment_number}.pkl')
            distributed_sketch_B_df = pd.DataFrame(distributed_sketch_B)
            distributed_sketch_B_df.to_pickle(
                f'./distributed_sketch_data/mbit={m_bit}_sketchtype={sketch_type}_proportion={proportion}_numprovider={num_provider}_{distribution}_numelement={num_elements}_distributed_sketchB_provider={provider_id}_trial={trial}_experimentnumber={experiment_number}.pkl')
            distributed_sketch_C_df = pd.DataFrame(distributed_sketch_C)
            distributed_sketch_C_df.to_pickle(
                f'./distributed_sketch_data/mbit={m_bit}_sketchtype={sketch_type}_proportion={proportion}_numprovider={num_provider}_{distribution}_numelement={num_elements}_distributed_sketchC_provider={provider_id}_trial={trial}_experimentnumber={experiment_number}.pkl')
            distributed_sketch_D_df = pd.DataFrame(distributed_sketch_D)
            distributed_sketch_D_df.to_pickle(
                f'./distributed_sketch_data/mbit={m_bit}_sketchtype={sketch_type}_proportion={proportion}_numprovider={num_provider}_{distribution}_numelement={num_elements}_distributed_sketchD_provider={provider_id}_trial={trial}_experimentnumber={experiment_number}.pkl')
    ...
    print(f"the average shuffle distance of {num_trials} trials is:")
    print(np.mean(np.array(shuffle_distance_all)))
    return 0


def create_equal_independent_data_set(num_unique, num_sample):
...
if __name__ == "__main__":
    ...
    experiment_MPS(num_element_sample, int(num_element_sample * proportion), distribution,epsilon,delta,num_trials,m_bit, experiment_number,  num_provider=num_provider)
